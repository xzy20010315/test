<!-- 用户已发布文章的展示页组件 -->
<template>
  <div class="wrapper" v-show="itemSpace">
    <div v-show="status">
    ...
    <div v-show="!status">
      <el-table :data="blogsDataDefault.slice((currentPage - 1) * pageSize, currentPage * pageSize)" stripe style="width: 100%"
      @row-click="showBlog">
      <el-table-column type="index" :index="indexMethod" width="40" />
      <el-table-column type="selection" prop="select" label="Select" width="40" />
      <el-table-column prop="blogTitle" label="BlogTitle" width="180" />
    ...
    <el-pagination class="el-pag" v-model:current-page="currentPage" v-model:page-size="pageSize"
      :page-sizes="[5, 10, 15, 20]" :small="small" :disabled="disabled" :background="background"
      layout="total, sizes, prev, pager, next, jumper" :total="blogsCountDefault" @size-change="handleSizeChange"
      @current-change="handleCurrentChange" />
    </div>
  </div>
</template>
...
import { findAllArgu, blogContentShowData } from '@/data/blog';
import { status } from '@/data/storage';
//导入方法


const itemSpace = ref(false)
const blogsDataDefault = ref([])
const blogsCountDefault = ref(0)
...
const showBlog = (row, event, column) => {
  ...
  bus.emit('sendMsg',"blogContentShow")
}
...
bus.on('sendMsg', id => {
  if (id == "itemSpace") {
    itemSpace.value = true
  } else {
    itemSpace.value = false
  }
})
